...
def produce_midi_file(data, bpm, start_time, vel_min, vel_max, instruments):
    ...
    for index, row in data.iterrows():
        ...
        if (current_pressure_category == 'high'): # spiele 2 Töne pro duration
            midi.addNote(1, 1, midi_data - 8, start_time, 1.0*duration, volume -10)
            midi.addNote(1, 1, midi_data - 8, start_time + 1.0*duration, 1.0*duration, volume -10)
        else: # spiele einen TOn pro duration
            midi.addNote(1, 1, midi_data - 8, start_time, duration, volume -10)

        midi.addNote(2, 2, harmony_note, start_time, 1.0*duration, volume - 10)  # Spur 1, Kanal 1, leiserer Ton
        ...
        midi.addNote(3, 3, midi_data - 32, start_time, 1.0*duration, volume_bass)
