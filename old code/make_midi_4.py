...
def produce_midi_file(data, bpm,  vel_min, vel_max, instruments):
    ...
    for index, row in data.iterrows():
        ...
        y_data = map_value(temp, -20, 50, 0, 1) # ??? include a season or something?
        y_data = map_value(temp, -15, 0, 0, 1) # ??? include a season or something?
        ...
        #w_data = map_value(wind_speed, 20, 100, 0, 1)
        ...
        if (current_pressure_category == 'high'): # spiele 2 Töne pro duration
            ...
            midi.addNote(1, 1, midi_data - 8, start_time, 1.1*duration, volume -10)
