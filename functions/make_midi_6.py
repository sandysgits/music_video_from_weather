...
def produce_midi_file(data, bpm, vel_min, vel_max, instruments):
    ...
    for _, row in data.iterrows():
        ...
        wind_speed = abs(row['FF_10']) if abs(row['FF_10']) <= 100 else 100 #todo dew to windspeed ! also in video!
        ...
        # temperature
        midi.addNote(1, 1, midi_data - 8, start_time, 0.5, volume + 10)
        midi.addNote(1, 1, midi_data - 8, start_time + 0.5, 0.5, volume + 10)
        
        midi.addNote(2, 2, midi_data + 8, start_time, 1.1, volume - 10)
        
        volume_bass = max(0, min(
            volume + 10 if pressure < 980 else
            volume + 5 if pressure < 1013.25 else
            volume - 10 if pressure > 1040 else
            volume - 5 if pressure >= 1013.25 else
            volume, 127
        ))
        
        midi.addNote(3, 3, midi_data - 32, start_time, 0.1, volume_bass)
